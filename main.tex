...
\renewcommand{\alert}[1]{\textbf{\color{red} #1}}
...
\hline
\hline
Methods that do not use domain knowledge & B and C   \\ 
Methods that use target relations & B, C and D  \\
Methods that learn target relations & B and C \\
Methods that use target representations & B, C and D \\
Methods that learn target representations & B and C \\
Matrix completion and hybrid methods & A \\
%Hybrid filtering methods & 1-5, 9 \\
%Structured output prediction methods ? & 1-4, 5*, 7-8, 11 \\
%Methods that assume deterministic dependencies ? & 1-4 \\ 
%Methods that optimize complex loss functions? &1-4, 8* \\
\hline  
\end{tabular}
\end{center}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Methods that do not use domain knowledge (B and C) \\
Example: Mean-regularized multi-task learning\footnote{Evgeniou and Pontil, Regularized multi--task learning, KDD 2004.}}
...
\begin{frame}{Methods that use target relations (B, C and D) \\
Example: Graph-based regularization\footnote{Gopal and Yang, Recursive regularization for large-scale classification with hierarchical
...
%\begin{frame}
%\frametitle{Stacking applied to multi-target prediction: general principle\footnote{\bibentry{Cheng_Hullermeier_2009}}}
%
%\begin{center}
%\def\layersep{2.5cm}
...
%\end{center}
%
%% Cheng and Huellermeier, Combining Instance-based learning and Logistic Regreession for Multi-Label classification, Machine Learning, 2009
%
%
%\end{frame}

\begin{frame}{Methods that learn target relations (B and C) \\
Example: learning target hierarchies\footnote{Rangwala and Naik, Tutorial on Large-Scale Hierarchical Classification, KDD 2017.}}

\includegraphics[width=\textwidth]{pics/hierarchies}

\begin{itemize}
\item It might be difficult for a human expert to define a hierarchy
...
\begin{frame}{Methods that use target representations (B, C and D) \\
Example: Kronecker kernel ridge regression\footnote{Waegeman et al., A kernel framework for learning graded relations from data, IEEE Transaction on Fuzzy Systems, 2012}}
...
\end{equation*}
Kronecker product pairwise kernel in the dual:
\begin{eqnarray*} 
...
\section{A specific multi-target prediction method}
...
\begin{frame}{Cross-validation: four different ways}
\vspace{-0.6cm}
 \begin{columns}[c]
\column{0.6\textwidth}
  \center 
 
   \includegraphics[height=0.9\textheight]{Figures/CVSettings}
\column{0.4\textwidth}
\begin{itemize}
\item Analytic shortcuts can be derived to perform LOOCV for each setting!

\item \alert{Tuning $\lambda_d$ and $\lambda_t$ essentially free!}
\end{itemize}
\end{columns}

\end{frame}

\begin{frame}{Effect of regularization for the four settings}
Data: protein-ligand interactions

Evaluation by AUC (\textcolor{orange}{lighter} = better performance)
 \center
   \includegraphics[width=\textwidth]{Figures/reg_landscape}
   
Clear difference between four settings and $\lambda_d$ and $\lambda_t$!
\end{frame}

\begin{frame}{Learning with mini-batches}
\begin{columns}[c]
\column{0.6\textwidth}
  \center 
   \includegraphics[height=0.8\textheight]{Figures/online}
\column{0.4\textwidth}
Exact updating of the parameters when new training instances and/or taks become available
\begin{itemize}
\item scalable for ``Big Data'' applications 
\item updating model in dynamic environment
\end{itemize}
\end{columns}
\end{frame}
