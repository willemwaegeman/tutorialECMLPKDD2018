...
\Ensure{Learnt $p \times m$ matrix $\vec{W}$ in sparse format} \pause 

\State Load single copy of input vectors $\vec{X} = \{\vec{x}_1 \ldots \vec{x}_n \}$ in the main memory  %\Comment{\textit{Refactor data without replication}} 
...
\State \Return $p \times m$ weight matrix $\vec{A}$ 
...
\end{columns}
\begin{itemize}
\item Of the 3 Billion weights, 97\% are s.t, $W_{d',m'} \leq |0.01|$, and hence non-discriminative
...
\item When $m$ is sufficiently enough
