...
% i.e.,  each row of the score matrix $Y$ is a permuation of $\{1,...,m\}$. 
...
\toprule
%&  &  &  \includegraphics[width=1cm]{pics/car_1.png} & \includegraphics[width=1cm]{pics/car_2.png} &  \includegraphics[width=1cm]{pics/car_3.png}\\
& $X_1$ & $X_2$ & $Y_1$ & $Y_2$ & $Y_3$ \\
\midrule
$\bx_1$ & 5.0 & 4.5 & 1 & 3 &  2 \\
$\bx_2$ & 2.0 & 2.5 & 2 & 1 &  3 \\
...
$$
L_r(\vec{y},\mathbf{h}(\vec{x})) = \sum_{(i,j): y_i > y_j}  \left (\llbracket h_i(\vec{x}) < h_j(\vec{x}) \rrbracket + \frac{1}{2} \llbracket h_i(\vec{x}) = h_j(\vec{x}) \rrbracket  \right )
$$
...
\item
\emph{Theorem}: A ranking function that sorts the labels according to their probability of relevance, i.e., using the scoring
function $\mathbf{h}(\cdot)$ with
$$
h_i(\vec{x}) \, = \, \Pr(Y_i = 1 \, | \, \vec{x}) \,,
$$
